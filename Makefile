...
V4L2_SRCS := $(APPS_DIR)/v4l2_server.cpp \
             $(SRC_DIR)/v4l2_device.cpp \
             $(SRC_DIR)/v4l2_video_source.cpp

LIBUSB_SRCS := $(APPS_DIR)/libusb_server.cpp
...
ifeq ($(UNAME_S),Darwin)
    ...
    APP_INCLUDES += $(patsubst -I%,-isystem %,$(OSSL_CFLAGS))
    OSSL_LDFLAGS := $(shell pkg-config --libs-only-L openssl 2>/dev/null || echo "-L/opt/homebrew/opt/openssl@3/lib")
endif

USOCKETS_LIB := third_party/uSockets/uSockets.a
APP_LDFLAGS  := -Wl,--start-group \
                $(USOCKETS_LIB) \
                $(BUILD_DIR)/libuseeplus.a \
...
$(LIBUSB_APP): $(BUILD_DIR)/libuseeplus.a $(GENERATED_HPP) | $(BUILD_DIR)
	@echo "Building libusb server..."
	$(CXX) $(CXXFLAGS) $(APP_INCLUDES) $(LIBUSB_SRCS) $< -o $@ $(APP_LDFLAGS)

$(V4L2_APP): $(V4L2_SRCS) $(BUILD_DIR)/libuseeplus.a $(GENERATED_HPP) | $(BUILD_DIR)
	@echo "Building V4L2 server..."
	$(CXX) $(CXXFLAGS) $(APP_INCLUDES) $(V4L2_SRCS) -o $@ $(APP_LDFLAGS)
